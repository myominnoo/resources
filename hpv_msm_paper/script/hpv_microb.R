...
hpv <- data.sd %>% 
  mutate(
    ...
    HIV_code1 = relevel(HIV_code1, "Yes"), 
...
table(hpv$HIV)
table(hpv$HIV, hpv$HPV_pos)

# remove all non-MSM ------------------------------------------------------

hpv <- hpv %>% 
  filter(MSM.STATUS.x == "MSM") %>% 
  mutate(
    HPV_pos = factor(HPV_pos, levels = c(1,0), labels = c("Positive", "Negative")), 
    HIV = factor(HIV), 
    HIV = relevel(HIV, "Positive")
  )
...
t1_hpv <- hpv %>%
  ...
  modify_spanning_header(c("stat_1", "stat_2") ~ "**HPV**")
...
t3_total <- hpv %>%
  tbl_summary(
    include = c(
      S.WORKER, D.PROST, num_sex_partner,
      all_partner, REGULAR_partner, CASUAL_partner, P.P, 
      oral_partner, ORAL_sex_reg, ORAL_sex_cas, ORAL_sex_pay, 
      insert_partner, INSERT_sex_reg, INSERT_sex_cas,
      INSERT_sex_pay, recep_partner, RECEPT_sex_reg, 
...
t1 <- tbl_stack(
  list(
    tbl_merge(tbls = list(t1_total, t1_hpv, t1_hiv), 
              tab_spanner = c("Total", "Any HPV", "HIV")), 
    tbl_merge(tbls = list(t2_total, t2_hpv, t2_hiv), 
              tab_spanner = c("Total", "Any HPV", "HIV")), 
    tbl_merge(tbls = list(t3_total, t3_hpv, t3_hiv), 
              tab_spanner = c("Total", "Any HPV", "HIV"))
  )
) %>% 
  as_flex_table()  %>%
  set_caption("Baseline characteristics by HIV Status")
t1
...
genotype <- genotype_all %>% 
  left_join(
    hpv %>% 
      select(PID, HIV, MSM.STATUS.x, age_cat), 
    by = "PID"
  ) %>% 
  mutate(value = as.numeric(value), 
         type = factor(value), 
         ...
         # HIV = ifelse(is.na(HIV), "Missing", HIV),
         hpv = case_when(
           value %in% hr_hpv_types ~ "HR-HPV", 
           value %in% lr_hpv_types ~ "LR-HPV", 
           value %in% cancer_hpv ~ "PO-HPV", 
           TRUE ~ NA_character_
         )) 
...
fig1a <- genotype %>% 
  ...
  summarize(n = n()) %>% 
  ggplot(aes(reorder(type, -n), n, fill = `Valent Vaccine`)) + 
  geom_bar(stat = "identity", width = 0.7) + 
  xlab("HPV Type") + 
  scale_y_continuous(name = "Number", breaks = seq(0, 12, 2)) + 
  facet_grid(~ hpv, scales = "free_x", space = "free") + 
  theme_classic()

fig1b <- genotype %>% 
  group_by(hpv, HIV, type) %>% 
  summarize(n = n()) %>% 
  ggplot(aes(reorder(type, -n), n, fill = HIV)) + 
  geom_bar(stat = "identity", width = 0.7) + 
  xlab("HPV Type") + 
  scale_y_continuous(name = "Number", breaks = seq(0, 12, 2)) + 
  facet_grid(~ hpv, scales = "free_x", space = "free") + 
...
fig1c <- genotype %>% 
  group_by(hpv, age_cat, type) %>% 
  summarize(n = n()) %>% 
  ggplot(aes(reorder(type, -n), n, fill = age_cat)) + 
  geom_bar(stat = "identity", width = 0.7) + 
  xlab("HPV Type") + 
  scale_y_continuous(name = "Number", breaks = seq(0, 12, 2)) + 
  facet_grid(~ hpv, scales = "free_x", space = "free") + 
...
genotype <- hpv %>% 
  select(PID, AGE, S.WORKER, HIV, age_cat,
         B.YEAR,	AGE, AGE.F.SEX,	M.STATUS,	EDU.LEVEL, 
  ...
  mutate(
    ...
    M.STATUS = relevel(M.STATUS, "Single"),
...
t4_total <- genotype %>% 
  # filter(any_hpv == 1) %>% 
  tbl_summary(
    include = c(any_hpv, multi_hpv, any_lr_hpv, 
...
t4 <- tbl_merge(tbls = list(t4_total, t4_hiv, t4_age), 
                tab_spanner = c("HPV Positivity", "HIV Status", "Age")) %>%
  as_flex_table()  %>%
  set_caption("Anal HPV genotype distributions stratified by HIV status and age")
t4




# change HIV reference level
genotype <- genotype %>% 
  mutate(HIV = relevel(HIV, "Negative"))
...
t5_total_anyHPV <- genotype %>%
  tbl_summary(
    by = any_hpv, 
    include = c(AGE, age_cat, M.STATUS, EDU.LEVEL, HIV, STI_RESULT2, 
                S.WORKER, num_sex_partner, all_partner, 
                insert_partner, recep_partner, condom_partner), 
    statistic = list(all_continuous() ~ "{median} ({p25}, {p75})",
                     all_categorical() ~ "{n} ({p})"),
    digits = list(
...
t5_cOR_anyHPV <- tbl_stack(list(
  glm_uni("any_hpv", "AGE", genotype, "**cOR (95% CI)**"),
...
glm(any_hpv ~ age_cat + M.STATUS + EDU.LEVEL + HIV + STI_RESULT2 + num_sex_partner + all_partner, data = genotype, family = binomial)
glm(any_hpv ~ age_cat + M.STATUS + HIV, data = genotype, family = binomial)

t5_aOR_anyHPV <- glm_uni("any_hpv", 
                         paste0("age_cat + M.STATUS + HIV + num_sex_partner"),
                         genotype, "**aOR (95% CI)**")
...
t5_cOR_anyHRhpv <- tbl_stack(list(
  glm_uni("any_hr_hpv", "AGE", genotype, "**cOR (95% CI)**"),
...
t5_aOR_anyHRhpv <- glm_uni("any_hr_hpv", 
                           paste0("age_cat + M.STATUS + EDU.LEVEL + HIV"),
                         genotype, "**aOR (95% CI)**")
...
genotype <- genotype %>% 
  mutate(any_val4 = ifelse(is.na(any_val4), 0, any_val4), 
         any_val9 = ifelse(is.na(any_val9), 0, any_val9)) 


t5_total_anyval9 <- genotype %>%
  tbl_summary(
    by = any_val9, 
    include = c(AGE, age_cat, M.STATUS, EDU.LEVEL, HIV, STI_RESULT2, 
                S.WORKER, num_sex_partner, all_partner, 
...
t5_cOR_anyval9 <- tbl_stack(list(
  glm_uni("any_val9", "AGE", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "age_cat", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "M.STATUS", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "EDU.LEVEL", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "HIV", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "STI_RESULT2", genotype, "**cOR (95% CI)**"),
  # glm_uni("any_val9", "MSM.STATUS.x", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "S.WORKER", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "num_sex_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "all_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "insert_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "recep_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val9", "condom_partner", genotype, "**cOR (95% CI)**")
))
t5_aOR_anyval9 <- glm_uni("any_val9", 
                          paste0("age_cat + EDU.LEVEL + HIV + STI_RESULT2 + ", 
                                 "num_sex_partner + all_partner"),
                           genotype, "**aOR (95% CI)**")
t5_anyval9 <- tbl_merge(list(t5_total_anyval9, t5_cOR_anyval9, t5_aOR_anyval9), 
                         tab_spanner = FALSE)
t5_anyval9



t5_total_anyval4 <- genotype %>%
  tbl_summary(
    by = any_val4, 
    include = c(AGE, age_cat, M.STATUS, EDU.LEVEL, HIV, STI_RESULT2, 
                S.WORKER, num_sex_partner, all_partner, 
...
t5_cOR_anyval4 <- tbl_stack(list(
  glm_uni("any_val4", "AGE", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "age_cat", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "M.STATUS", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "EDU.LEVEL", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "HIV", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "STI_RESULT2", genotype, "**cOR (95% CI)**"),
  # glm_uni("any_val4", "MSM.STATUS.x", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "S.WORKER", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "num_sex_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "all_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "insert_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "recep_partner", genotype, "**cOR (95% CI)**"),
  glm_uni("any_val4", "condom_partner", genotype, "**cOR (95% CI)**")
))
t5_aOR_anyval4 <- glm_uni("any_val4", 
                          paste0("age_cat + EDU.LEVEL + HIV + STI_RESULT2 + ", 
                                 "num_sex_partner + all_partner"),
                          genotype, "**aOR (95% CI)**")
t5_anyval4 <- tbl_merge(list(t5_total_anyval4, t5_cOR_anyval4, t5_aOR_anyval4), 
                        tab_spanner = FALSE)
...
t6 <- tbl_merge(list(t5_anyval9, t5_anyval4), 
                tab_spanner = c("Valent-9 vaccine preventable", 
                                "Valent-4 vaccine preventable")) %>%
  as_flex_table()  %>%
  set_caption("S: Regression analyses for HPV-associated risk factors")
t6 
